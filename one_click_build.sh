...
# 使用最新的 Amazon Linux 2 AMI for ap-northeast-2
...
# 检查用户是否已修改 GitHub 仓库链接
if [[ "$GITHUB_REPO_URL" == "<请在这里替换成你的私有GitHub仓库链接>" ]]; then
    echo "❌ 错误: 请先编辑 one_click_build.sh 文件，将 <请在这里替换...> 替换成你自己的 GitHub 仓库链接。"
    exit 1
fi

# 定义一个清理函数，用于在脚本退出时（无论成功或失败）自动执行
cleanup() {
    echo "--- 正在清理所有临时创建的 AWS 资源... ---"
    # 使用 'aws ec2 describe-instances' 检查实例是否存在
    if aws ec2 describe-instances --instance-ids "$INSTANCE_ID" --region "$REGION" --output text &>/dev/null; then
    ...
    aws ec2 delete-key-pair --key-name "$KEY_NAME" --region "$REGION" &>/dev/null && echo "✅ 密钥对已删除。"
    aws ec2 delete-security-group --group-id "$SG_ID" --region "$REGION" &>/dev/null && echo "✅ 安全组已删除。"
    aws iam remove-role-from-instance-profile --instance-profile-name "$PROFILE_NAME" --role-name "$ROLE_NAME" --region "$REGION" &>/dev/null
    aws iam delete-instance-profile --instance-profile-name "$PROFILE_NAME" --region "$REGION" &>/dev/null && echo "✅ 实例配置文件已删除。"
    aws iam detach-role-policy --role-name "$ROLE_NAME" --policy-arn arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryFullAccess --region "$REGION" &>/dev/null
    aws iam delete-role --role-name "$ROLE_NAME" --region "$REGION" &>/dev/null && echo "✅ IAM 角色已删除。"
...
until ssh -q -o "StrictHostKeyChecking no" -o "ConnectTimeout=10" -i "${KEY_NAME}.pem" ec2-user@"$PUBLIC_IP" "exit"; do
    ...
    if [ $RETRY_COUNT -ge $MAX_RETRIES ]; then
        echo "❌ SSH 连接失败，已达到最大重试次数。请检查你的网络设置或AWS安全组规则。"
        exit 1
    fi
...
REMOTE_SCRIPT="
    ...
    # EC2实例角色会自动提供凭证，我们只需要配置区域
    aws configure set default.region ${REGION}

    echo '--- (云端) 正在构建并推送 Docker 镜像... 这会花费几分钟时间，请耐心等待。 ---'
    # 使用 sudo 运行 docker，因为用户组权限需要重新登录才能生效
